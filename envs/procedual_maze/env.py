...
class Maze(gym.Env):
    ...
    def step(self, action):
        ...
        reward = 1.0 if terminated else -0.01 if not invalid_move else -1
...
class MazeV2(gym.Env):
    def __init__(self, 
                 width=5, 
                 height=5,
                 step_limitation:int=10_000,
                 dataset:np.ndarray=None,
                 ):
        '''
        Args:
            step_limitation: The maximum number of steps allowed per episode.
        '''
        self.logger = logging.getLogger(__name__)
    ...
    def step(self, action):
        ...
        self.step_elapsed += 1
        truncated = self.step_elapsed >= self.step_limitation
        # print(f"[W] After trailing in a same maze {self.step_limitation} time, trucate it to go to a new maze...")
...
def generate_maze(seed: int = 42, split_ratio=0.2, width=4, height=4) -> Tuple[np.ndarray, np.ndarray]:
    '''
    Here is the description of the maze generation process:
        https://ljp0vuj4fr1r.jp.larksuite.com/wiki/QyQJwfSRBiuvXZk8kXpjTxKmpEe?from=from_copylink
    ...
    Cell meanings:
        ...
        3 = goal
    '''
    original_set = pathfinding(width, height)
    original_training_set, original_evaluation_set = train_test_split(original_set, test_size=split_ratio, random_state=seed)

    training_mazes = maze_variation(width, height, original_training_set, random_state=seed)
    evaluation_mazes = maze_variation(width, height, original_evaluation_set, random_state=seed)

    return training_mazes, evaluation_mazes

def pathfinding(width, height) -> np.ndarray:
    """
    Generate every maze whose open cells form one unique player-goal path.
    ...
    Returns:
        3D numpy array of shape (num_mazes, height, width) containing the generated mazes.
    """
    return generate_all_single_solution_mazes(width, height)

def train_test_split(mazes: np.ndarray, test_size: float = 0.2, random_state: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    """
    Split the generated mazes into training and evaluation sets.
